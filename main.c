...
int main(int argc, char **argv)
{
    ...
    module->description = _("Displays user-specified raster maps in the active "
                            "graphics frame.");

    /* set up command line */
    map = G_define_standard_option(G_OPT_R_MAPS);
    map->description = _("Name of raster map(s) to be displayed");
    ...
    while(1) {
        ...
        G_message("Render raster map layer %s",(name));
